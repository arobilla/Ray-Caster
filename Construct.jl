...
function buildkinkedstandard(ang, r, l)
    ang *= pi/180
        if ang == 0
        pt1 = [Geom.Point(0,0), Geom.Point(2*l+2*2*l, 0)]
        pt2 = [Geom.Point(0, 2*r), Geom.Point(2*l+2*2*l, 2*r)]
    else
        pt1 = [Geom.Point(0, 2*r),
        Geom.Point(l + r*(-tan(ang/2)), 2*r),
        Geom.Point(l + 2*l*cos(ang), (2*l*cos(ang) + r*sin(ang))*tan(ang)+r*(1+ cos(ang))),
        Geom.Point(l+2*2*l*cos(ang) - r*(-tan(ang/2)), 2*r),
        Geom.Point(2*l+2*2*l*cos(ang), 2*r)]

        pt2 = [Geom.Point(0, 0),
        Geom.Point(l+r*tan(ang/2), 0),
        Geom.Point(l+2*l*cos(ang), (2*l*cos(ang) - r*sin(ang))*tan(ang) + r*(1-cos(ang))),
        Geom.Point(l+2*2*l*cos(ang)-r*(tan(ang/2)), 0),
        Geom.Point(2*l + 2*2*l*cos(ang), 0)]
    end
    line_segs = []
    ...
    by = [0, (2*l*cos(ang)+r*sin(ang))*tan(ang) + r*(1+cos(ang))]
